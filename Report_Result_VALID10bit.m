...
addpath('features\')
addpath('SVR\')
load VALID10bit_all_mos
load VALID10bit_MPT_MWI_BLiF_PCA

scene_num = 5;
c_num = 0;
lower=-1; upper=1;
input_NR = [features];
output = VALID10bit_all_mos;
...
for i = 1:5
    input_code_NR{i,:}=input_NR((i-1)*20+1:i*20,:);  
    output_code{i,:}=output((i-1)*20+1:i*20,:);
end
for i=0:8
    for j=-8:0
        ...
        for fork=1:all_num
            ...
            for m=1:size(test',1)
                input_test_NR(1+20*(m-1):(20*m),:)=input_code_NR{test(m)};
                output_test(1+20*(m-1):(20*m),:)=output_code{test(m)};
            end
            for n=1:size(train',1)
                input_train_NR(1+20*(n-1):(20*n),:)=input_code_NR{train(n)};
                output_train(1+20*(n-1):(20*n),:)=output_code{train(n)};
            end
